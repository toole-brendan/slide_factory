"""slide1_outsourced_bc_walk — Distributed Shipbuilding New Construction deck (20260610), source slide 1.

Walk from total ship spend down to outsourced Basic Construction, for two
program families (DDG-51 left, Virginia/Columbia submarines right), paired with
a step-by-step rationale ledger. Anatomy, in paint order (back to front),
mirrors the layer sections in ``_body()``:

  1. tables ............ rationale ledger (right) + walk title bar (left)
  2. walk charts ....... two native stacked-bar waterfalls (``CHARTS`` -> rId2
                         submarine, rId3 DDG)
  3. step rules ........ dashed per-step tick marks (``STEP_TICKS_*``) + the
                         coloured outsourced-BC end caps
  4. chart labels ...... footnote marker, program chips, ``IN_YEAR_LABELS``
  5. source line
  6. callouts .......... annualized outsourced-BC ``~$1.1B`` / ``~$3.0B`` badges
  7. walk axis ......... centre divider, ``WALK_STEP_LABELS``, prime AP/LLTM label

Each native walk chart is built by ``_walk_chart()`` from transcribed source
values (no converter sidecars). Repeated shapes are frozen-dataclass lists, each
emitted by a one-line helper. This is a hand-restructured version of the
converter output; the emitted OOXML is unchanged.
"""
...
TEACHING_METADATA = {
    ...
    "use_when": (
        ...
        "deductions into outsourced basic construction for two program families."
    ),
    "rebuild_strategy": (
    ...
    "teaches": (
        ...
        "manual step labels shared across paired charts",
...
# Right chart in the source slide: Virginia / Columbia-class submarine walk.
SUBMARINE_WALK_DATA = {
    "offset_values": [None, 71.05496661892461, 60.565462798924614, None, 18.133846531924611, None, None],
    "total_values": [90.223471970000006, None, None, 60.565462798924614, None, None, 18.133846531924611],
    "reduction_values": [None, 19.168505351075396, 10.489503819999996, None, 42.431616267000003, None, None],
    "value_axis_max": 90.223471970000006,
}

# Left chart in the source slide: DDG-51 walk. AP/LLTM is a patterned segment
# on the total-spend bar, matching the source chart part.
DDG_WALK_DATA = {
...
_CHART0_DATA = {
    ...
    "series": [
        {"name": "Hidden offsets", "values": SUBMARINE_WALK_DATA["offset_values"]},
        {"name": "Total / subtotal / outsourced BC", "values": SUBMARINE_WALK_DATA["total_values"]},
        {"name": "Removal steps", "values": SUBMARINE_WALK_DATA["reduction_values"]},
    ],
}
...
CHARTS = [
    _walk_chart(**SUBMARINE_WALK_DATA),
...
# ── shared overlay geometry (EMU) ──
STEP_TICK_H = IN(0.269)                                  # dashed step-rule length
IN_YEAR_Y, IN_YEAR_W, IN_YEAR_H = IN(2.009), IN(0.875), IN(0.186)
STEP_LBL_X, STEP_LBL_W, STEP_LBL_H = IN(0.495), IN(1.531), IN(0.241)


# Dashed per-step tick rules, top→bottom, one per walk-step transition.
STEP_TICKS_SUBMARINE = [
    StepTick("Straight Connector 16", 7.523, 2.696),
    StepTick("Straight Connector 24", 6.924, 3.302),
    StepTick("Straight Connector 25", 6.595, 3.908),
    StepTick("Straight Connector 40", 6.595, 4.516),
    StepTick("Straight Connector 42", 5.267, 5.122),
    StepTick("Straight Connector 43", 5.267, 5.727),
]

STEP_TICKS_DDG = [
    StepTick("Straight Connector 27", 4.469, 2.698),
    StepTick("Straight Connector 28", 3.51, 3.304),
    StepTick("Straight Connector 36", 3.34, 3.91),
    StepTick("Straight Connector 37", 3.34, 4.517),
    StepTick("Straight Connector 38", 2.127, 5.123),
    StepTick("Straight Connector 39", 2.127, 5.729),
]
...
def _body() -> str:
    ...
    out.append(table(n(), "StepRationaleLedger", IN(8.131), IN(1.385), IN(4.703), IN(5.332), col_widths=[IN(1.401), IN(3.301)], rows=[
        ...
        trow([cell("Total Ship Spend", size=PT(9), bold=True, color=WHITE, fill="79838F", T=edge(BLACK), B=edge(WHITE, 6350)), rcell([tpara([trun("In-Year Spend:", size=PT(9), bold=True, underline=True, color=BLACK, font=FONT), trun(" FY2022–FY2027 SCN end cost (P-5c discretionary); OBBBA adds two FY2026 DDG-51s ($5.4B) and a second FY2026 ", size=PT(9), color=BLACK, font=FONT), trun("Virginia", size=PT(9), italic=True, color=BLACK, font=FONT), trun(" $4.6B)", size=PT(9), color=BLACK, font=FONT)], bullet=True, mar_l=142875, indent=-142875), tpara([trun("Advanced Procurement / Long-Lead Time Materials:", size=PT(9), bold=True, underline=True, color=BLACK, font=FONT), trun(" ", size=PT(9), bold=True, color=BLACK, font=FONT), trun("DDG-51 adds incremental FY2026 AP/LLTM ($1.8B); submarines add none (supplier long-lead is already inside Basic Construction)", size=PT(9), color=BLACK, font=FONT)], bullet=True, mar_l=142875, indent=-142875)], T=edge(BLACK), B=edge("808080", 6350))], h=IN(1.2)),
        ...
        trow([cell("Less: Prime BC", size=PT(9), bold=True, color=WHITE, fill="486D82", T=edge(WHITE, 6350), B=edge(WHITE, 6350)), rcell([tpara([trun("Removes construction the Primes perform in their own yards.", size=PT(9), color=BLACK, font=FONT)], bullet=True, mar_l=142875, indent=-142875, line_spacing=115000), tpara([trun("BC spend x the announced outside-yard share from DoD award place-of-performance data: ~25% DDG-51 (FY22 ships 22%), 34% ", size=PT(9), color=BLACK, font=FONT), trun("Virginia", size=PT(9), italic=True, color=BLACK, font=FONT), trun(" / 22% ", size=PT(9), color=BLACK, font=FONT), trun("Columbia", size=PT(9), italic=True, color=BLACK, font=FONT), trun(".", size=PT(9), color=BLACK, font=FONT)], bullet=True, mar_l=142875, indent=-142875, line_spacing=115000)], T=edge("808080", 6350), B=edge("808080", 6350))], h=IN(0.788)),
        trow([cell("Less: Prime AP/LLTM", size=PT(9), bold=True, color=WHITE, fill="1D4D68", T=edge(WHITE, 6350), B=edge(WHITE, 6350)), rcell([tpara([trun("DDG-51:", size=PT(9), bold=True, underline=True, color=BLACK, font=FONT), trun(" The P-10 EOQ base is vendor-purchased material, so Primes retain none — $0 removed; the full $1.0B flows to Outsourced BC.", size=PT(9), color=BLACK, font=FONT)], bullet=True, mar_l=142875, indent=-142875, line_spacing=115000), tpara([trun("Submarines:", size=PT(9), bold=True, underline=True, color=BLACK, font=FONT), trun(" No AP/LLTM stream exists, so there is nothing to remove.", size=PT(9), color=BLACK, font=FONT)], bullet=True, mar_l=142875, indent=-142875, line_spacing=115000)], T=edge("808080", 6350), B=edge("808080", 6350))], h=IN(0.711)),
    ...
    # ── Layer 2 · native waterfall walk charts (rId2 submarine, rId3 DDG) ──
    out.append(graphic_frame(sp_id=n(), name="Chart", x=IN(4.609), y=IN(1.979), cx=IN(3.003), cy=IN(4.736), rId="rId2"))
    out.append(graphic_frame(sp_id=n(), name="Chart", x=IN(1.556), y=IN(1.981), cx=IN(3.003), cy=IN(4.736), rId="rId3"))

    # ── Layer 3 · dashed per-step tick rules + coloured outsourced-BC end caps ──
    for _s in STEP_TICKS_SUBMARINE:
        out.append(_step_tick(n, _s))
    out.append(text_box(n(), "Rectangle 44", IN(5.264), IN(5.391), IN(0.007), IN(0.337), [paragraph([], align="ctr", line_spacing=100000)], fill="79838F", line_color="none", anchor="ctr"))
    for _s in STEP_TICKS_DDG:
        out.append(_step_tick(n, _s))
    out.append(text_box(n(), "Rectangle 48", IN(2.123), IN(5.392), IN(0.007), IN(0.337), [paragraph([], align="ctr", line_spacing=100000)], fill="1D4D68", line_color="none", anchor="ctr"))

    # ── Layer 4 · chart annotations (footnote marker, program chips, in-year labels) ──
    out.append(text_box(n(), "Text Placeholder 25", IN(4.372), IN(2.446), IN(0.115), IN(0.167), [paragraph([run("1", size=PT(10), font=FONT)], align="ctr", mar_l=0, indent=0, line_spacing=100000)], fill=WHITE, line_color="none", anchor="ctr", wrap="none", vert="horz", l_ins=17463, t_ins=0, r_ins=17463, b_ins=0))
    out.append(text_box(n(), "ProgramChip", IN(2.238), IN(1.736), IN(2.078), IN(0.217), [paragraph([run("DDG-51", size=PT(9), bold=True, italic=True, color=BLACK, font=FONT)], align="ctr")], fill=None, line_color=BLACK, anchor="ctr", l_ins=0, t_ins=0, r_ins=0, b_ins=0))
    out.append(text_box(n(), "ProgramChip", IN(5.416), IN(1.736), IN(2.078), IN(0.217), [paragraph([run("Virginia / Columbia-class", size=PT(10), bold=True, italic=True, color=BLACK, font=FONT)], align="ctr")], fill=None, line_color=BLACK, anchor="ctr", l_ins=0, t_ins=0, r_ins=0, b_ins=0))
    for _l in IN_YEAR_LABELS:
        out.append(_in_year_label(n, _l))

    # ── Layer 5 · source line ──
    out.append(text_box(n(), "Sources", IN(0.495), IN(6.696), IN(12.339), IN(0.346), [paragraph([run("Sources: Navy SCN P-5c / P-40 budget justification, FY22–FY27, and PB27 FYDP outyears (FY28–FY31); FY26, PL 119-21 Sec. 20002; PB27 SCN, Columbia P-10 Strategic Outsourcing narrative; OUSD(C) Green Book Procurement deflators; Navy Shipbuilding Plan; PB27 SCN Exhibit P-10, LI 2122 (AP/LLTM Ship Construction EOQ); DoD contract award announcements (announced place-of-performance)", size=PT(8), color=DK, font=FONT)], line_spacing=100000)], fill=None, line_color="none", l_ins=91440, t_ins=45720, r_ins=91440, b_ins=45720))

    # ── Layer 6 · annualized outsourced-BC callouts ──
    out.append(text_box(n(), "AnnualizedCallout", IN(3.273), IN(5.714), IN(1.258), IN(0.47), [paragraph([run("~$1.1B", size=PT(14), bold=True, color=BLACK, font=FONT)], align="ctr", space_after=20), paragraph([run("annualized", size=PT(9), italic=True, color=BLACK, font=FONT)], align="ctr")], fill=WHITE, line_color=BLACK, anchor="ctr", l_ins=0, t_ins=0, r_ins=0, b_ins=0))
    out.append(text_box(n(), "AnnualizedCallout", IN(6.663), IN(5.714), IN(1.258), IN(0.47), [paragraph([run("~$3.0B", size=PT(14), bold=True, color=BLACK, font=FONT)], align="ctr", space_after=20), paragraph([run("annualized", size=PT(9), italic=True, color=BLACK, font=FONT)], align="ctr")], fill=WHITE, line_color=BLACK, anchor="ctr", l_ins=0, t_ins=0, r_ins=0, b_ins=0))

    # ── Layer 7 · walk axis (centre divider, step labels, prime AP/LLTM label) ──
    out.append(connector(n(), "Straight Connector 18", IN(4.584), IN(2.231), IN(0), IN(4.205), color="808080", width=6350, dash="dash", flip_v=True))
...
CHROME = Chrome(
    ...
    takeaway=(
        ...
        "of outsourced work for DDG-51 and ~$3.0B for submarines."
    ),
    preliminary=True,
)
