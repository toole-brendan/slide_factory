"""awards_methodology - <deck purpose; fill in>.
...
Two dirs go on sys.path so both packages resolve regardless of entry point:
  - the build dir (this package's parent) so ``awards_methodology`` resolves;
...
_BUILD_DIR = str(_HERE.parents[1])   # slide_factory/projects/awards_methodology/ (build dir)
